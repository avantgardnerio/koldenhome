...
const persistValue = (nodeId, args) => {
  ...
  insertEvent(nodeId, prop, args.newValue ?? args.value ?? null, args.commandClass).catch((err) => {
...
export const wireDriverEvents = (driver, manager) => {
  driver.on("driver ready", () => {
    ...
    ctrl.on("node added", (node, result) => {
      logEvent("controller", "node added", { nodeId: node.id, result });
      wireNodeEvents(node);
    });
