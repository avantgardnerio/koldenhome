...
export async function getEnabledCameras() {
  const { rows } = await pool.query(
    "SELECT stream_id, name FROM cameras WHERE enabled = true ORDER BY name",
  );
  return rows;
}
...
export async function insertEvent(nodeId, property, value, commandClass) {
  await pool.query(
    "INSERT INTO events (time, node_id, property, value, command_class) VALUES (NOW(), $1, $2, $3, $4)",
    [nodeId, property, JSON.stringify(value), commandClass ?? null],
  );
}

// ─── Plot Data ──────────────────────────────────────────────────────────────

export async function getPlotData(nodeIds, properties, days = 7) {
  const { rows } = await pool.query(
    `SELECT node_id, time, property,
            CASE WHEN value::text IN ('true','false')
                 THEN CASE WHEN value::text = 'true' THEN 1 ELSE 0 END
                 ELSE value::text::float
            END AS value,
            command_class
     FROM events
     WHERE node_id = ANY($1)
